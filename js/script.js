...
function updateSpitter(event) {
  ...
  if (rgbtype.includes('x')) {
    if (rgbtype.includes('Â§')) {
      output = othercolorsout.join('').replace(/&/g, 'Â§');
    } else {
  ...
  document.getElementById('graylabel2').innerText = 'This is what you put in the chat. Click on it to copy.';
  showError();
